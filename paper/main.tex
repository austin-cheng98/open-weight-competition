...
\newcommand{\ChangeRate}{3.4}
\newcommand{\NeverRepriced}{56}
\newcommand{\CutShare}{60}
\newcommand{\SurvSix}{55}
\newcommand{\SurvTwelve}{43}
\newcommand{\PanelObs}{18{,}649}
\newcommand{\PanelModels}{587}
\newcommand{\PanelStart}{November 2023}
\newcommand{\PanelEnd}{March 2026}
\newcommand{\NRevisions}{659}
\newcommand{\NPriceEvents}{306}
\newcommand{\ElastTok}{$-0.51$}
\newcommand{\ElastTokSE}{0.11}
\newcommand{\ElastReq}{$-0.44$}
\newcommand{\ElastReqSE}{0.10}
\newcommand{\ElastCross}{$-0.23$}
...
back to 2023 with archived catalogues. Three facts emerge. Posted prices are almost
never revised, so competition works through releases rather than repricing: only
\ChangeRate\% of catalogue observations follow a price change and \NeverRepriced\% of
models never change price. When prices do change, volume responds weakly, with an
...
model version is close to a fixed characteristic of the product: \NeverRepriced\% of
models never change price and only \ChangeRate\% of catalogue observations follow a
change, a rigidity far above what is observed in retail goods
\citep{nakamura2008,cavallo2018}. Second, when prices do change, volume moves little.
...
\subsection{Posted prices are almost never revised}

The archived catalogues let us follow the posted price of a model version from its
first appearance. Between consecutive catalogue observations, a median of five days
apart, the price changes \ChangeRate\% of the time. Over a model's observed life,
...
This rigidity is extreme by the standards of the price-setting literature, where
monthly repricing frequencies of ten to thirty percent are typical
\citep{bils2004,nakamura2008,cavallo2018}. The explanation offered there, menu costs,
does not apply: changing a number in an API catalogue is nearly free. What is missing
instead is the incentive, which the next fact supplies. Whatever the cause, the
consequence for measurement is immediate. A research design that looks for the
competitive effect of open weights in the price series of incumbent models is looking
at a variable that hardly moves.
...
The right panel of Figure~\ref{fig:prices} traces the dynamics around \NRevisions{}
...
\caption{\textbf{Prices do not move, and demand barely responds when they do.} Left:
...
\midrule
$\log$ price & $-0.510$ & $-0.438$ & $-0.554$ & $-0.517$ & $-0.230$ \\
             & $(0.105)$ & $(0.089)$ & $(0.119)$ & $(0.208)$ & $(0.080)$ \\
\midrule
Observations & 18{,}649 & 18{,}649 & 8{,}345 & 10{,}304 & 18{,}649 \\
Models       & 587 & 587 & 299 & 291 & 587 \\
\bottomrule
\end{tabular}
\end{table}
...
\exp(-\lambda d_{jk})$ with $d_{jk}$ the Mahalanobis distance between capability
...
(Figure~\ref{fig:cf} and Table~\ref{tab:cfgrid}). The price effect is almost invariant
...
standard deviations of 16.5, 22.6 and 18.5. Imputation raises coverage from 109 models
...
same-provider release, which covers 36\% of the panel, leaves the estimate at
$-0.509$; dropping those observations gives $-0.495$ (s.e.\ 0.125).
...
\midrule
Fixed-weight price (baseline) & $-0.510$ & 0.105 & 18{,}649 & 587 \\
Input price                   & $-0.501$ & 0.099 & 18{,}649 & 587 \\
Output price                  & $-0.466$ & 0.113 & 18{,}649 & 587 \\
Realised blended price        & $-0.627$ & 0.098 & 18{,}649 & 587 \\
Requests rather than tokens   & $-0.438$ & 0.089 & 18{,}649 & 587 \\
Weekly captures only          & $-0.472$ & 0.123 & 15{,}133 & 432 \\
Trailing captures only        & $-0.384$ & 0.144 & 3{,}516  & 375 \\
Open-weight models            & $-0.554$ & 0.119 & 8{,}345  & 299 \\
Proprietary models            & $-0.517$ & 0.208 & 10{,}304 & 291 \\
2025 onwards                  & $-0.475$ & 0.107 & 6{,}752  & 495 \\
\bottomrule
\end{tabular}
\end{table}
...
parameter. The price effect is nearly invariant to $\sigma$, because it is driven by
